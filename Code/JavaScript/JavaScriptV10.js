...
function init() {
	...
	scene.add( mesh );
	// Player
	var mass = 100, radius = 10;
	var sphereShape = new CANNON.Sphere(radius); // Step 1
	var player = new CANNON.RigidBody(mass,sphereShape); // Step 2
	player.position.set(0,0,0);
	world.add(player); // Step 3
...
function animate() {
	...
	for (var i = 0; i < objects.length; i++) {
		...
		if (objects[i].objectType === "Cube") {
			if ( isOnObject === true ) {
				if (checkX(player.position.x, objects[i].position.x, objects[i].geometry.parameters.width)) {
					//velocity.y = Math.max( 0, velocity.y );
					//canJump = true;
					//player.position.y = objects[i].position.y + (objects[i].geometry.parameters.height + 5);
					console.log("Worked X");	
				} if (checkY(player.position.y, objects[i].position.y, objects[i].geometry.parameters.height)) {
					//player.position.x  = objects[i].position.x + 20.5;
					console.log("Worked Y");
				} if (checkZ(player.position.z, objects[i].position.z, objects[i].geometry.parameters.depth)) {
					console.log("Worked Z");
				}
			}
		}
