...
function init() {
	...
	for (var i = 0; i < level.buttons.length; i++) {
		...
		buttonMesh[i] = new THREE.Mesh(geometry,material);
		buttonMesh[i].position.x = level.buttons[i].PosX;
		buttonMesh[i].position.y = level.buttons[i].PosY;
		buttonMesh[i].position.z = level.buttons[i].PosZ;
		scene.add(buttonMesh[i]);
	}
	for (var i = 0; i < level.obstacles.length; i++) {
		...
		obstacleMesh[i] = new THREE.Mesh(geometry,material);
		obstacleMesh[i].position.x = level.obstacles[i].PosX;
		obstacleMesh[i].position.y = level.obstacles[i].PosY;
		obstacleMesh[i].position.z = level.obstacles[i].PosZ;
		scene.add(obstacleMesh[i]);
	}
	for (var i = 0; i < level.enemies.length; i++) {
		if (level.enemies[i].type === 'Thumb') {
			geometry = new THREE.SphereGeometry(5,16,16);
			material = new THREE.MeshBasicMaterial( {color: 0x00ffff, side: THREE.FrontSide});
			enemyMesh[i] = new THREE.Mesh(geometry,material);
			enemyMesh[i].position.x = level.enemies[i].PosX;
			enemyMesh[i].position.y = level.enemies[i].PosY;
			enemyMesh[i].position.z = level.enemies[i].PosZ;
...
function animate() {
	...
	for (var i = 0; i < buttonMesh.length; i++) {
		...
		buttonMesh[i].rotation.y += 5.0 * delta;
	}
